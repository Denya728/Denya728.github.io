(function(){
  ...
  function paidAmount(q){return Math.max(0,(Number(q.total)||0)-(Number(q.balance)||0));}\n  async function createOnlinePayment(id){\n    const q=(state.quotes||[]).find(x=>x.id===id);if(!q)return;\n    if(!window.DENYAPayments?.call)return toast("Módulo de pagos no disponible.");\n    const suggested=Math.max(1,Number(q.depositAmount)||((Number(q.total)||0)*(Number(q.deposit)||50)/100));\n    const raw=prompt("Monto a cobrar online en MXN:",String(suggested.toFixed(2)));\n    if(raw===null)return;const amount=Number(raw);if(!amount||amount<=0)return toast("Monto inválido.");\n    try{const r=await window.DENYAPayments.call("create_checkout",{amount,quotation_id:q.id,folio:q.folio,customer_name:q.client,customer_email:q.email||"",success_url:location.origin+location.pathname+"?payment=success&session_id={CHECKOUT_SESSION_ID}",cancel_url:location.origin+location.pathname+"?payment=cancelled"});\n      q.payment_link_url=r.url;q.payment_provider="stripe";q.payment_status="pending";q.paymentAmount=amount;save();\n      if(navigator.clipboard)await navigator.clipboard.writeText(r.url).catch(()=>{});\n      toast("Link de pago creado y copiado.");openDetail(id);\n    }catch(e){toast(e?.message||"No se pudo crear el pago.")}\n  }\n  window.v60CreateOnlinePayment=createOnlinePayment;
  ...
  function openDetail(id){
    ...
    bg.innerHTML=`<div class="v60-detail-panel">
      ...
      <div class="v60-detail-actions"><button class="primary" onclick="v60CloseDetail();newQuote('${q.id}')">Editar</button>${typeof quotePdf==='function'?`<button class="secondary" onclick="quotePdf('${q.id}')">PDF</button>`:''}<button class="secondary" onclick="v60DuplicateQuote('${q.id}')">Duplicar</button><button class="primary" onclick="v60CreateOnlinePayment('${q.id}')">Cobrar online</button>${typeof quoteWhatsApp==='function'?`<button class="ghost" onclick="quoteWhatsApp('${q.id}')">WhatsApp</button>`:''}</div>
