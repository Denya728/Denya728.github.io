(function(){
  ...
  window.setProfilePlan=function(name){if(!profilePlans[name])return;state.subscription=state.subscription||{};state.subscription.plan=name;state.plan=name;const t=profileTemplates.find(x=>x.id===state.quoteTemplate);if(t&&!allowedTemplate(t))state.quoteTemplate='minimal';save();renderProfile('plan');toast('Plan cambiado a '+name)};
  ...
  function planPane(){const cur=currentPlan();return `<div class="profile-section"><h3>Plan actual</h3><div class="profile-plan-grid">${Object.entries(profilePlans).map(([name,p])=>`<div class="profile-plan-card ${cur===name?'active':''}"><h4>${name}</h4><div class="price">$${p.price} <small>/ mes</small></div><div class="hint">${p.users} usuario${p.users===1?'':'s'} · ${p.templates} plantilla${p.templates===1?'':'s'} · ${p.brands} marca${p.brands===1?'':'s'}</div><button class="${cur===name?'secondary':'primary'}" onclick="setProfilePlan('${name}')">${cur===name?'Plan actual':'Cambiar a '+name}</button></div>`).join('')}</div><div class="helper" style="margin-top:12px">Cambiar el plan en esta demo actualiza inmediatamente los límites de plantillas y usuarios.</div></div>`}
