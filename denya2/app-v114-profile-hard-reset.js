...
`;
  ...
  function account(){var s=window.state&&state.subscription||{},plan=s.plan||'Negocio';return shell('account','Plan y suscripción','Aquí está el estado de tu cuenta y tu plan.','<div class="grid"><section class="card"><div class="ey">Plan actual</div><h3>'+esc(plan)+'</h3><p>Tu suscripción está asociada a tu cuenta de SWEETLAB.</p><div class="price">$449 <small>/ mes</small></div><span class="badge">Activo</span></section><section class="card"><div class="ey">Facturación</div><h3>Suscripción</h3><p>Administra ciclo, plan y datos de facturación.</p><button class="primary" id="114manage">Administrar suscripción</button></section></div><section class="card" style="margin-top:15px"><h3>Planes disponibles</h3><p>Elige el nivel que mejor corresponda a tu operación.</p><div class="plans">'+[['Emprende','$249'],['Negocio','$449'],['Pro','$699']].map(function(x){return '<div class="plan '+(x[0]===plan?'current':'')+'"><div class="ey">'+(x[0]===plan?'ACTUAL':'PLAN')+'</div><h3>'+x[0]+'</h3><div class="price">'+x[1]+' <small>/ mes</small></div><button class="secondary" data-114-plan="'+x[0]+'">'+(x[0]===plan?'Plan actual':'Ver plan')+'</button></div>'}).join('')+'</div></section>')}
  ...
  function bind(tab){
    ...
    if(tab==='account'){var m=document.getElementById('114manage');if(m)m.onclick=function(){if(window.openSubscription)window.openSubscription();};}
