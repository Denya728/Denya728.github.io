(function(){
  ...
  window.openMeasure=function(id=null,productId=''){
    ...
    let requirements=clone(reqsOf(m));if(!requirements.length)requirements=[{kind:'base',recipeId:'',qty:1}];requirements=requirements.map(r=>({...r,recipeId:r.recipeId||''}));
    ...
    const w=modal(id?'Editar presentación':'Nueva presentación',`
      ...
      <div class="section"><div class="page-head" style="margin:0"><div><h3>Consumo de esta presentación</h3><div class="hint">Aquí eliges exactamente qué receta usa esta presentación y cuánto consume.</div></div><button class="secondary" id="addReq">+ Agregar consumo</button></div><div id="reqBox"></div></div>
    `,wrap=>{
      sync();const pr=getProduct(wrap.querySelector('#mproduct').value),piece=ptype(pr)==='piece';
      const saleUnit=wrap.querySelector('#msaleunit').value, data={id:id||'m'+Date.now(),productId:wrap.querySelector('#mproduct').value,name:wrap.querySelector('#mname').value.trim(),saleUnit,minPeople:saleUnit==='people'?(Number(wrap.querySelector('#mmin')?.value)||''):'',maxPeople:saleUnit==='people'?(Number(wrap.querySelector('#mmax')?.value)||''):'',yieldPieces:saleUnit!=='people'?(Number(wrap.querySelector('#myield')?.value)||1):'',requirements:requirements.filter(r=>r.kind&&r.recipeId&&Number(r.qty)>0),components:[],materials:[]};
    ...
    function sync(){w.querySelectorAll('[data-req-row]').forEach(row=>{const i=Number(row.dataset.reqRow);requirements[i]={kind:row.querySelector('[data-rkind]').value,recipeId:row.querySelector('[data-rrecipe]').value,qty:Number(row.querySelector('[data-rqty]').value)||0}})}
    function draw(){w.querySelector('#reqBox').innerHTML=requirements.map((r,i)=>{const recs=recipesFor(r.kind),selected=r.recipeId||recs[0]?.id||'';return `<div class="extra-row" data-req-row="${i}" style="display:grid;grid-template-columns:1.05fr 1.7fr .8fr 42px;gap:8px;margin:9px 0"><select data-rkind>${Object.entries(kindMeta).map(([v,x])=>`<option value="${v}" ${v===r.kind?'selected':''}>${x.label}</option>`).join('')}</select><label class="field" style="margin:0"><span>Receta</span><select data-rrecipe>${recs.map(rec=>`<option value="${rec.id}" ${rec.id===selected?'selected':''}>${esc(rec.name)}</option>`).join('')||'<option value="">Sin recetas disponibles</option>'}</select></label><label class="field" style="margin:0"><span>Cantidad</span><input data-rqty type="number" step="0.05" min="0" value="${r.qty||0}"></label><button class="icon" data-rrm="${i}" title="Quitar consumo">×</button></div>`}).join('');w.querySelectorAll('[data-rkind],[data-rrecipe],[data-rqty]').forEach(el=>el.onchange=el.oninput=()=>{sync();if(el.matches('[data-rkind]'))draw()});w.querySelectorAll('[data-rrm]').forEach(b=>b.onclick=()=>{requirements.splice(Number(b.dataset.rrm),1);draw()})}
    w.querySelector('#addReq').onclick=()=>{sync();requirements.push({kind:'filling',recipeId:'',qty:.5});draw()};
