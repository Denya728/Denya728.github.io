(function(){
  ...
  function ensureStyle(){if(document.getElementById('v34merge'))return;const s=document.createElement('style');s.id='v34merge';s.textContent=`
    .v34-section{margin-top:18px}.v34-title{display:flex;justify-content:space-between;align-items:end;gap:12px;margin:0 0 10px}.v34-title h3{margin:2px 0 0;font-size:18px}.v34-mini{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin-bottom:14px}.v34-mini .card{padding:16px}.v34-mini strong{display:block;font:700 25px Georgia,serif;margin:6px 0}.v34-ops{display:grid;grid-template-columns:1.35fr 1fr;gap:14px}.v34-row{display:grid;grid-template-columns:82px 1fr auto;gap:10px;align-items:center;padding:10px 0;border-bottom:1px solid #eee5de}.v34-row:last-child{border-bottom:0}.v34-alert{display:flex;justify-content:space-between;gap:12px;padding:10px 0;border-bottom:1px solid #eee5de}.v34-alert:last-child{border-bottom:0}.v34-lock{background:#fbf8f4;border:1px dashed #d8c7b8;border-radius:14px;padding:14px;color:#7d695d}@media(max-width:900px){.v34-ops{grid-template-columns:1fr}}@media(max-width:650px){.v34-mini{grid-template-columns:1fr}.v34-row{grid-template-columns:70px 1fr}.v34-row .badge{grid-column:2;justify-self:start}}
  ...
  views.home=function(){
    ...
    const revenueAll=sold.reduce((s,q)=>s+(Number(q.total)||0),0);
    ...
    const low=(state.inventory||[]).filter(i=>i.active!==false&&Number(i.stock)<=Number(i.stockMin??5)).slice(0,8);
    const block=has('Negocio')?`<div class="v34-section"><div class="v34-title"><div><div class="v33-eyebrow">Panorama general</div><h3>Lo que ya tenías, conservado</h3></div><span class="badge ok">${esc(plan())}</span></div><div class="v34-mini"><div class="card"><div class="muted">Pedidos por entregar</div><strong>${accepted.length}</strong><div class="hint">Saldo pendiente ${money(outstanding)}</div></div><div class="card"><div class="muted">Ventas cerradas acumuladas</div><strong>${sold.length}</strong><div class="hint">Total vendido ${money(revenueAll)}</div></div><div class="card"><div class="muted">Cotizaciones abiertas</div><strong>${quotes.filter(q=>q.status==='Borrador').length}</strong><div class="hint">Seguimiento comercial</div></div></div><div class="v34-ops"><div class="v33-card"><div class="v33-eyebrow">Operación diaria</div><h3>Próximos pedidos</h3>${upcoming.length?upcoming.map(q=>`<div class="v34-row"><b>${esc(fmtDate(q.event))}</b><div><b>${esc(q.client||'Cliente')}</b><div class="hint">${esc(getProduct(q.productId)?.name||'Pedido')}</div></div><span class="badge ok">${money(q.total)}</span></div>`).join(''):'<div class="empty">No hay pedidos aceptados por entregar.</div>'}</div><div class="v33-card"><div class="v33-eyebrow">Inventario</div><h3>Alertas operativas</h3>${low.length?low.map(i=>`<div class="v34-alert"><span>${esc(i.name)}</span><div style="text-align:right"><b>${Number(i.stock).toFixed(2)} ${esc(i.useUnit||i.unit||'')}</b><div class="hint">Mín. ${Number(i.stockMin||0).toFixed(2)}</div></div></div>`).join(''):'<div class="helper">Inventario sin alertas críticas.</div>'}</div></div></div>`:`<div class="v34-section v34-lock"><b>Operación diaria</b><br>Próximos pedidos, saldos pendientes y alertas operativas están disponibles desde Negocio.</div>`;
