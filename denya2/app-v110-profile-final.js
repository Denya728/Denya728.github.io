...
(function(){
  ...
  async function render(tab='company'){
    ...
    else if(tab==='customization'){const old=window.DENYAProfileV108?.render; if(old){await old('customization'); const h=document.getElementById('content')?.querySelector('.v108-shell')?.innerHTML; if(h){document.getElementById('content').innerHTML=shell('customization','<div class="v110-legacy">'+h+'</div>');bindTabs('customization');return}} body='<div class="v110-empty">No se pudo cargar Personalización.</div>'}
