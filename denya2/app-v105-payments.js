...
(function(){
  ...
  async function call(action,p={}){const x=c();if(!x)throw new Error("Supabase no disponible");const {data:{session}}=await x.auth.getSession();if(!session)throw new Error("Tu sesión expiró. Vuelve a iniciar sesión.");const r=await x.functions.invoke("denya-payments-v2",{body:{action,organization_id:org(),...p}});if(r.error)throw new Error(r.error.message||"Error de conexión");if(r.data?.error)throw new Error(r.data.error);return r.data}
  ...
  async function renderPayments(){
    ...
    try{
      ...
      async function refresh(){
        ...
        const ds=await call("disputes");document.getElementById("v105Disputes").innerHTML=(ds.disputes||[]).length?(ds.disputes||[]).map(d=>'<div class="v105-row"><div><b>'+esc2(d.reason||"Disputa")+'</b><div class="v105-muted">'+money(Number(d.amount||0)/100)+' · '+esc2(d.status)+'</div></div><span class="v105-status">'+esc2(d.status)+'</span></div>').join(""):'<div class="v105-empty">No hay disputas activas.</div>';
      }
