(function(){
  ...
  openMeasure=function(id=null,productId=''){
    const m=id?getMeasure(id):{productId:productId||state.products.find(p=>p.active)?.id||'',name:'',minPeople:'',maxPeople:'',requirements:[]};
    ...
    const w=modal(id?'Editar presentación':'Nueva presentación',`
      ...
      <div class="form2">
        ...
        ${field('Personas máximas (opcional)','mmax',m.maxPeople||'','number')}
      </div>
      <div class="section">
  ...
  newQuote=function(editId=null){
    ...
    function loadMeasures(){
      ...
      renderDynamic();renderExtras();calc();
    }
    function renderDynamic(){
      ...
      w.querySelector('#qpeople .helper').textContent=m.minPeople||m.maxPeople?`${m.minPeople||0}–${m.maxPeople||0} personas`:'No aplica';
      const reqs=reqsOf(m);
    ...
    function calculateQuote(){
      const p=getProduct(psel.value),m=getMeasure(msel.value);let cost=p.pricing==='fixed'?0:presentationCost(m,selections,packages);let subtotal=p.pricing==='fixed'?(Number(p.price)||0):cost/(1-(Number(p.margin)||50)/100);
