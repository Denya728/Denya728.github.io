(function(){
  ...
  newQuote=function(editId=null){
    ...
    function renderDynamic(){
      ...
      const unit=(m.saleUnit||(/(^|\s)(pz|pieza|piezas|unidad|unidades|pack|paquete|paquetes)(\s|$)/i.test(m.name||'')?'pz':'people'));
    ...
    function calculateQuote(){
      const p=getProduct(psel.value),m=getMeasure(msel.value),unit=(m?.saleUnit||(/(^|\s)(pz|pieza|piezas|unidad|unidades|pack|paquete|paquetes)(\s|$)/i.test(m?.name||'')?'pz':'people')),qty=unit==='people'?1:Math.max(1,Number(w.querySelector('#qquantity')?.value)||1);let cost=p.pricing==='fixed'?0:presentationCost(m,selections,packages);cost*=qty;let subtotal=p.pricing==='fixed'?(Number(p.price)||0)*qty:cost/(1-(Number(p.margin)||50)/100);
